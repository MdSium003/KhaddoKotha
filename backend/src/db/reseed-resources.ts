...
async function reseedResources() {
  try {
    console.log("🔄 Reseeding resources table...");

    // Delete all existing resources
    await sql`DELETE FROM resources`;
    console.log("   ✓ Deleted existing resources");

    // Insert new resources data
    const resources = [
    ...
    console.log(`   ✓ Inserted ${resources.length} new resources`);

    console.log("\n✅ Resources reseeded successfully!");
  } catch (error) {
    console.error("❌ Error reseeding resources:", error);
    process.exit(1);
  }
}
