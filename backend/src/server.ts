...
app.post("/api/diet-plan", async (req, res) => {
  try {
    ...
    const body = z.object({
      ...
      calorieGoal: z.enum(["low", "medium", "high"]),
      mealPreference: z.enum(["vegetarian", "non-vegetarian", "vegan", "any"]),
      additionalComments: z.string().optional(),
    }).parse(req.body);

    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });

    const inventoryList = body.inventoryItems && body.inventoryItems.length > 0
      ? `Available inventory items: ${body.inventoryItems.join(", ")}`
      : "No inventory items specified";

    const calorieRanges = {
      low: "1200-1500 calories per day",
      medium: "1500-2000 calories per day",
      high: "2000-2500 calories per day"
    };

    const systemPrompt = `You are a professional nutritionist and meal planner. Create a complete daily diet plan with the following specifications:

${inventoryList}
${body.includeExternal ? "You can suggest items not in the inventory." : "Try to use only items from the inventory when possible."}
Calorie Goal: ${calorieRanges[body.calorieGoal]}
Meal Preference: ${body.mealPreference}
${body.dietaryRestrictions ? `Dietary Restrictions: ${body.dietaryRestrictions}` : ""}
${body.additionalComments ? `Additional Requirements: ${body.additionalComments}` : ""}
...
- List of ingredients (3-6 items)
- Brief cooking instructions (2-3 sentences)
- Approximate nutrition info (calories, protein, carbs, fats)
...
Return ONLY a valid JSON object in this exact format (no markdown, no code blocks):
{
  "breakfast": {
    ...
    "nutritionInfo": "Calories: X, Protein: Xg, Carbs: Xg, Fats: Xg"
  },
  "lunch": {
    "name": "Meal Name",
    "ingredients": ["ingredient1", "ingredient2", "ingredient3"],
